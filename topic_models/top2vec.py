...
def printWordBar(model, numTopic):
    ...
    df_probs = pd.DataFrame(topic_probs).rename(columns={0: "Probability"})
    df = pd.concat([df_topics, df_probs], axis=1)
    fig = px.bar(df, y="Probability", x="Topic Words", text_auto=".2s", title= "Topic " + str(numTopic))
    fig.update_traces(
        textfont_size=12, textangle=0, textposition="outside", cliponaxis=False
    )
    return fig
...
def get_top_documents_Top2Vec(df, model, num_topics, k):
    ...
    for topic_num in range(0, num_topics):
        documents, document_scores, document_ids = model.search_documents_by_topic(topic_num = topic_num, num_docs = k)
...
def get_top_documents_Top2Vec_reduced(df, model, num_topics, k):
    ...
    topic_words = []
    for topic_num in range(0, num_topics):
        documents, document_scores, document_ids = model.search_documents_by_topic(topic_num = topic_num, num_docs = k, reduced = True)
        for index, doc in enumerate(documents):
            if index > k:
                break
            sample = df.loc[df['processed'] == doc]['text'].values[0]
